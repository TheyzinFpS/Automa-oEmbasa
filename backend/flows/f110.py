...
def _normalizar_contexto(cliente, doc_fat):
    cliente_normalizado = str(cliente or "").strip()
    doc_fat_normalizado = str(doc_fat or "").strip()

    if not cliente_normalizado:
        raise ValueError("Cliente ausente para preparar o F110.")

    if not doc_fat_normalizado:
        raise ValueError("Documento de faturamento ausente para preparar o F110.")

    return cliente_normalizado, doc_fat_normalizado


def _montar_contexto_mock(cliente, doc_fat):
    cliente_normalizado, doc_fat_normalizado = _normalizar_contexto(cliente, doc_fat)
    data_execucao = datetime.now().strftime("%d%m%Y")
    data_lancamento = _ultimo_dia_mes()
    identificacao = "BOL01"
    documento_selecao = f"00{doc_fat_normalizado}"

    return {
        "cliente": cliente_normalizado,
        "doc_fat": doc_fat_normalizado,
...
def _montar_dados_retorno(contexto):
    return {
        "boleto": "Mockado",
...
def f110(session, cliente, doc_fat, logger, progress_callback=None):
    ...
    try:
        ...
        return resultado_padrao(
            ...
            dados=_montar_dados_retorno(contexto),
        )
        ...
        notificar_progresso(
            ...
            "F110",
            "erro",
            "Falha ao executar o F110 mockado.",
        )
